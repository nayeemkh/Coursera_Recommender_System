...
struct friendTuple
{
    map<string,vector<string> >friendCourses;
};
struct query
{
    vector<string> myCourses;
    vector<friendTuple> friends;
};
struct ccbar
{
    double forClass;
...
void buildSWList()//build from a text file for stopwords,instead
{
...
int main()
{
    ...
    //}
    ClassFeatures* CF;
    CF=new ClassFeatures[7];
    for(int j=0;j<7;j++)
    {
        //trainScan<<classesMI[j].subject<<endl;
        int count=0;
        for(map<double,vector<string> > :: reverse_iterator mit=classesMI[j].MIscores.rbegin();mit!=classesMI[j].MIscores.rend() && count<1;mit++)
        {
            count++;
            vector<string> :: iterator vit;
            //trainScan<<mit->first<<endl;
            for(vit=mit->second.begin();vit!=mit->second.end();vit++)
            {
                //trainScan<<*vit<<" ";
                CF[j].features.push_back(*vit);
            }
            //trainScan<<endl;
        }
        //trainScan<<CF[j].features.size()<<endl;
        //trainScan<<endl;
    }
    int run;
    ifstream testData;
    cout<<"choose the run file please : 1,2,3"<<endl;
    cin>>run;
    //cout<<"run is :"<<run<<endl;
    if(run==1)
    {
        //cout<<"yes!"<<endl;
        testData.open("/home/sumit/Desktop/Assignment-2/UserProfiles.run#1.txt");
    }
    else if(run==2)
    {
        testData.open("/home/sumit/Desktop/Assignment-2/UserProfiles.run#2.txt");
    }
    else
    {
        testData.open("/home/sumit/Desktop/Assignment-2/UserProfiles.run#3.txt");
    }
    cout<<"yes!"<<endl;
    query runquery;
    string sub;
    getline(testData,sub);
    trainScan<<sub<<endl;
    if(sub=="User's Courses:")
    {
        while(1)
        {
            getline(testData,sub);
            ...
            {
                runquery.myCourses.push_back(sub);
            }
        }
    }
    getline(testData,sub);
    if(sub=="Friends' Courses:")
    {
        while(getline(testData,sub))
        {
            friendTuple tuple;
            if(sub=="FEND")
            {
                runquery.friends.push_back(tuple);
            }
            else
            {
                string key=sub;
                ...
                tuple.friendCourses[key]=classes;
                runquery.friends.push_back(tuple);
            }
        }
        trainScan<<runquery.myCourses.size()<<endl;
        trainScan<<runquery.friends.size()<<endl;
    }
    return 0;
}
